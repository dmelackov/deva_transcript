...
from deva_transcript.neural.utils import extract_audio_and_convert, extract_key_frames
...
async def task_transcribe(task_model: Task, session: Session, s3: S3_client, logger: Logger):
     ...
     with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            s3.fget_object(settings.minio_bucket, source_file.minio_name, str(input_path))
            ...
            s3.fput_object(settings.minio_bucket, new_file.minio_name, str(output_path))
...
async def task_summary(task_model: Task, session: Session, s3: S3_client, logger: Logger):
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        s3.fget_object(settings.minio_bucket, transcript_file.minio_name, str(input_path))
...
async def frames_extract_task(task_model: Task, session: Session, s3: S3_client, logger: Logger):
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        for i in images:
            ...
            if new_file is None:
                raise Exception("File not created")
            s3.fput_object(settings.minio_bucket, new_file.minio_name, str(i[1]))
